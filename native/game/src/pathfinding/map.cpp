...
void Map::setChunks(Chunks* chunks){
    ...
    this->chunks = chunks;
    this->width = (chunks->xMaxVoxels-chunks->xMinVoxels+1);
    this->depth = (chunks->zMaxVoxels-chunks->zMinVoxels+1);
    int size = width*depth;
    cells = new PathCell[size];
    //dmLogInfo("size:%d",size);
    int id = 0;
    for (int z = chunks->zMinVoxels; z <= chunks->zMaxVoxels; z++) {
        for (int x = chunks->xMinVoxels; x <= chunks->xMaxVoxels; x++) {
            PathCell cell(x,z, id);
            cell.blocked = this->chunks->getVoxel(x,64,z) == 0 ||
                this->chunks->getVoxel(x,65,z) != 0 || this->chunks->getVoxel(x,66,z) != 0;
            cells[id] = cell;
            id++;
        }
    }
