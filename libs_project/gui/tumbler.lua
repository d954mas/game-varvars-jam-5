...
function Btn:set_checked(checked)
	...
	local cb = GOOEY.checkbox(self.root_name, nil, nil, self.gooey_listener, self.refresh_checkbox)
	cb.set_checked(self.checked)
end

function Btn:on_input(action_id, action)
	if (not self.ignore_input) then
		local cb = GOOEY.checkbox(self.root_name, action_id, action, self.gooey_listener, self.refresh_checkbox)
		return cb.consumed
	end
end
