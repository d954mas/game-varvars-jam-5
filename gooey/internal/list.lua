...
local function get_instance(list_id, stencil_id, refresh_fn, lists)
	local hash_stencil = core.to_hash(stencil_id)
	local list = core.instance(hash_stencil, lists, LIST)
	list.id = list_id
	list.stencil_id = stencil_id

	list.scroll = list.scroll or vmath.vector3()
	list.stencil = list.stencil or gui.get_node(hash_stencil)
